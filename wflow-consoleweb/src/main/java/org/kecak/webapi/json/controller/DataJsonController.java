...
public class DataJsonController implements Declutter {
    ...
    public void getFormData(final HttpServletRequest request, final HttpServletResponse response,
        ...
        try {
            ...
            if (asAttachmentUrl) {
                formData.addRequestParameterValues(DataJsonControllerHandler.PARAMETER_AS_LINK, new String[]{"true"});
            }
    ...
    protected WorkflowProcessResult submitFormToStartProcess(PackageActivityForm packageActivityForm, FormData formData) {
        ...
        formData.addRequestParameterValues(AssignmentCompleteButton.DEFAULT_ID, new String[]{AssignmentCompleteButton.DEFAULT_ID});
        String appId = packageActivityForm.getPackageDefinition().getAppDefinition().getAppId();
        String version = packageActivityForm.getPackageDefinition().getAppDefinition().getVersion().toString();
        String processDefId = packageActivityForm.getProcessDefId();
        return appService.submitFormToStartProcess(appId, version, packageActivityForm, processDefId, formData, workflowVariables, null);
    }
