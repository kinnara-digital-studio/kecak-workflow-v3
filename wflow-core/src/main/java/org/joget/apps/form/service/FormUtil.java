...
public class FormUtil implements ApplicationContextAware {
    ...
    public static Stream<Element> elementStream(@Nonnull Element element, FormData formData) {
        ...
        // hide show section
        if (element instanceof Section && isHiddenSection((Section) element, formData)) {
            return Stream.empty();
        }
    ...
    protected static boolean isHiddenSection(Section section, FormData formData) {
        return !section.isMatch(formData);
    }
}